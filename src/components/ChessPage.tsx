...
import { Typography, AppBar, Button, Dialog, DialogTitle, DialogContent, DialogActions, makeStyles } from "@material-ui/core";
...
const ChessPage = () => {
    ...
    useEffect(() => {
        const socketInstance = io("https://13.233.65.153");
    ...
    useEffect(() => {
        ...
        if (socket) {
            ...
            socket.on("callUser", ({ from, name: callerName, signalData }: { from: any; name: any; callerName: any; signalData: any; }) => {
        ...
        peer.on("signal", (data) => {
            ...
            setTimeout(() => {
                if (callAccepted === false) {
                    ...
                    toast('The User You are trying to connect is not responding or Does not Exist, Please Crosscheck the connection id also')
                }
            }, 60000)
        });
    ...
    return (
        <div className='Container'>
            <div className={classes.wrapper}>
                ...
                <Options Me={Me} callAccepted={callAccepted} name={name} setname={setname} LeaveCall={LeaveCall} callUser={CallUser} callEnded={callEnded} IdToCall={IdToCall} setIdToCall={setIdToCall} callingUser={callingUser}>
            ...
            <Dialog open={connectionError}>
                ...
                <DialogContent>
                    <div>
                        Sorry, we couldn't establish a connection to the server. Please check your internet connection or try again later.
                    </div>
                </DialogContent>
                <DialogActions>
                    <a href='/' ><Button onClick={() => console.log('hfffhf')
                    }>OK</Button></a>
                </DialogActions>
            </Dialog>
        </div>
